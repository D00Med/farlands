...
local function furnace_node_timer_advanced(pos, elapsed)
	--
	-- Inizialize metadata
	--
	local meta = minetest.get_meta(pos)
	local fuel_time = meta:get_float("fuel_time")*1.5 or 0
	local src_time = meta:get_float("src_time")*0.5 or 0
	local fuel_totaltime = meta:get_float("fuel_totaltime")/2 or 0
	...
	while update do
		...
		for _, pos in ipairs(n) do
			...
			if minetest.get_item_group(item, "boiler") ~= 0 then
				water = true
			end
		end

		
		--Check if there is water and a vent
		if water and vent then
		-- Check if we have enough fuel to burn
		if fuel_time < fuel_totaltime then
			-- The furnace is currently active and has enough fuel
			fuel_time = fuel_time + elapsed
...
minetest.register_node("default:furnace2", {
	...
	on_timer = furnace_node_timer_advanced,
